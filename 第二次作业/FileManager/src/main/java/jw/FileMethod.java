...
public class FileMethod {
    ...
    public void show(String CurrentFile,String[] arg,String name)
    {
        String path = CurrentFile + File.separator +name;
        ...
        if (files != null) {
            if(arg.length==3){
                if (arg[2].equals("name")) {
                    // 按名称排序
                    Arrays.sort(files, Comparator.comparing(File::getName));
                    System.out.println("sort by name!");
                } else if (arg[2].equals("time")){
                    // 按最近修改时间排序
                    Arrays.sort(files, Comparator.comparingLong(File::lastModified));
                } else if (arg[2].equals("byte")) {
                    // 按文件大小排序
                    Arrays.sort(files, Comparator.comparingLong(File::length));
                    System.out.println("sorted by byte!");
                } else {
    ...
    public static File changeDirection(String str, File CurrentFile)
    {
        if(str.equals(".."))//上一级
        {
        ...
        else
        {
            ...
            if(absoluteFile.isAbsolute())
            {
                if (!absoluteFile.exists())
                    ...
                    System.out.println("请输入目录名称，非文件名称！");
            ...
            else
            {
                if (!relativeFile.exists())
                    ...
                    System.out.println("请输入目录名称，非文件名称！");
                else if (relativeFile.isDirectory())
    ...
    public void paste(String CurrentFile,String[] arg)
    {
        ...
        else if(arg.length==4&&arg[3].equals("front")&&flag)
        {//前台执行
            FileCopy.copyFile(copyFile.getAbsolutePath(),resultFile.getAbsolutePath());
        }
        else if(arg.length==4&&arg[3].equals("back")&&flag)
        {//后台执行，启动新建线程
    ...
    public void encrypt(String CurrentFile,String[] arg) throws IOException, NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException
    {
        String srcFile = CurrentFile + File.separator +arg[1];
        String destFile = CurrentFile + File.separator +arg[3];
        System.out.println("请输入加密密匙（1到16位，请勿输入同样的字符）:");
